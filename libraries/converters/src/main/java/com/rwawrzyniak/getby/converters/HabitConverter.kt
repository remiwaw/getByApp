...
class HabitConverter : Converter<HabitModel, Habit>(::convertModelToEntity, ::convertEntityToModel) {
	
	companion object{
		fun convertEntityToModel(entity: Habit): HabitModel {
			return HabitModel(
				entity.id,
				entity.name,
				entity.description,
		...
		fun convertModelToEntity(model: HabitModel): Habit {
			return Habit(
				model.id,
				model.name,
				model.description,
