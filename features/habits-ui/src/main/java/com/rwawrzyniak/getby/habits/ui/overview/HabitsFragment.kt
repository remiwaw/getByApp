	...
	override fun onCreateView(
		...
		binding.habitSearch.addTextChangedListener(object : TextWatcher {
			...
			override fun onTextChanged(text: CharSequence, start: Int, before: Int, count: Int) {
				simplySubscribe(viewModel.onAction(
					HabitsViewAction.OnTextFilterChanged(
						text.toString()
					)
				))
			}
	...
	override fun onOptionsItemSelected(item: MenuItem): Boolean {
		when (item.itemId) {
			...
			R.id.hideArchived -> {
				...
				simplySubscribe( viewModel.onAction(
					HabitsViewAction.OnShowArchiveChange(
				...
				if (event == DISMISS_EVENT_ACTION) {
					(binding.daysListView.adapter as HabitsAdapter).undo()
						...
						if (swipedAction == SwipedAction.REMOVE) {
							simplySubscribe( viewModel.onAction(
								HabitsViewAction.OnRemoveHabit(
									swipedHabit
								)
							))
						} else {
							simplySubscribe( viewModel.onAction(
								HabitsViewAction.OnSwitchArchiveState(
									swipedHabit
								)
							))
						}
			...
			when (direction) {
				...
				ItemTouchHelper.RIGHT -> {
					showUndoSnackbar(swipedItem,
						SwipedAction.CHANGE_ARCHIVE_STATE
					)
				}
			}
		}
