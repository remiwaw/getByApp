...
class HabitDetailsFragment : BaseFragment(), OnChartGestureListener {
	...
	override fun onStart() {
		...
		subscribeTo(viewModel.onAction(
			HabitDetailsViewAction.InitializeView(
				habitId
			)
		))
		historyCalendarSave.setOnClickListener {
			subscribeTo(viewModel.onAction(
				HabitDetailsViewAction.OnSaveCalendarClicked(
					binding.historyCalendar.selectedDates
				)
			))
		}
	}
	...
	private fun refreshData(){
		subscribeTo(viewModel.onAction(
			HabitDetailsViewAction.LowestVisibleXBecomesVisible(
				binding.lineChart.xAxis.axisMinimum.toInt()
			)
		))
	}
